...
- 操作前明确确认清空任务队列、处理日志、待审核草稿与解析/OCR/AI 中间产物；继续复用受管产物目录的 scoped cleanup，不删除源文档或已入库知识 Markdown。
- 清理后验证目录状态、刷新界面并如实报告删除的文件/目录数；取消不执行，重复清理报告 0，失败时不显示伪成功。
- 新增缓存维护回归，覆盖可见性、确认/取消、安全范围、幂等性和失败语义。

## v2.20.10 — 2026-08-03 当前运行最终可见清单

- 最终提交清单绑定 `task_id`、`run_id` 与事务，逐条记录 `final_path`、内容哈希、验证时间、Markdown 类型和目标库；只有文件与索引提交后经 Obsidian 公共 Vault API 重读通过，才成为 `visible_verified`。
- 完成状态、`cardsWritten`、任务统计和 UI 路径只取当前运行清单中唯一可见最终路径，并在成功前强制计划、提交、可见验证三集合恒等；临时/隐藏/源文件路径、旧运行证据和验证后失效路径均不计数。
- 启动恢复只接受同任务、同运行、同事务的 committed manifest；旧成功仅保留解析与模型检查点并重新验证或重写，不再按相同计划或旧清单升级为成功。
- 真实 Obsidian 双启动门升级为生产完成状态链：写计划、提交、最终 manifest、任务终态、UI 统计、逐文件打开与重启重验，并覆盖中文、日文、英文和空格路径。

## v2.20.9 — 2026-08-03 真实 Obsidian 写入门与单一写入端口

- 生产结构化提交、回滚与知识索引重建统一经过 `KnowledgeWritePort`，只以 Obsidian 公共 Vault API 可见且可读回的记录作为成功事实。
- 新增真实 Obsidian/Electron 双启动发布门，验证插件实例加载、中文/日文/空格路径写入与实际打开、重跑幂等、删除失效和注入式部分失败回滚。
- 发布证据同时绑定当前 Git commit、`main.js` SHA-256、插件版本与实际 Obsidian 窗口版本；假成功审计覆盖生产提交、回滚、索引重建和旧审核入口。
...
- 同一任务增加唯一执行所有者和可恢复租约。自动队列、单文件、选中任务和重试同时触发时只会执行一次，重复入口会提示“已在处理中”，不会覆盖控制器或终态。
- 所有 vault 路径边界拒绝 Windows 盘符、UNC/设备路径、POSIX 绝对路径、目录穿越、控制字符和空必填路径；合法中文、日文、英文、空格和 apostrophe 保持不变。旧路径迁移不再把危险原值重新放回可执行字段。
- 损坏的结构化索引、项目登记表和任务账本现在有明确错误代码、实际读取阶段和修复建议；插件不会静默清空或覆盖原文件，修复后可继续复用解析与统一知识检查点。
- 任务账本保存改为串行写入，并让待写快照立即对后续读取可见；进度防抖仍保留，终态、租约和产物边界会强制落盘，避免并发保存丢记录。
- 新增加载实际三文件 bundle 的可执行门禁，覆盖英文翻译缓存 miss/hit、并发任务入口、损坏状态、账本重叠保存和非破坏恢复；完整测试、类型检查、构建和基准均通过。
...
- 新增可扩展统一知识单元、区域级覆盖账本、遗漏区域定向修复、确定性缓存键及块/区域/单元/覆盖/压缩/调用/写入遥测；旧 atom/card 在边界保守规范化。
- 两库从文档级改为知识单元级路由；在办项目内容与通用/历史知识可从同一来源分流。项目归档使用稳定 ID、路径索引和移动计划保持链接有效。
- 新增受控标签同义词归一、显式身份关系和双向稳定 wikilink；禁止仅凭表面关键词连边。
- 四类 Markdown writer 接收统一模型，先计划后事务提交；来源定位改为中文表达。审核只保留实质冲突、整份文档归属、不安全写入和高影响关系歧义，并按根因合并。
- 新增跨 PDF-like/DOCX/XLSX/email/text 等价、混合用途、覆盖修复、迁移、幂等、归档、噪声、长密集输入及成本阈值端到端测试。
...
- 归并改为通用命题类型、内容派生主题词、数字/时间事实、模态、条件/例外、来源章节/块/表格/消息邻域及本地词法相似度；冲突数字、模态和条件不合并，并保留全部 atom/point/evidence provenance。
- 证据只回填源块中的逐字 span；本地保守中日英匹配保留数字/单位，并携带表格 sheet/range/row/column/header 与邮件 thread/message/from/date/subject 上下文。
- 数量异常降为文档级警告与三个定向样本，不再把整份文档逐卡送审；真实噪声、重复、不可制卡块和不安全视觉结论直接硬拒绝，审核仅保留可操作不确定项并输出稳定原因码。
- 新增“本地重新归并、校验并路由最近任务（零模型调用）”命令，复用 parsed/classification/summary/atoms，禁止 provider 调用，以 card_id 去重写入并迁移 review v2.0 审计。
- 诊断新增各阶段基数、归并/丢弃/审核/拒绝原因直方图、数量警告样本和 provider 增量计数；不记录正文、prompt、密钥或 provider 原始响应。
- 新增 fixture-independent 中英日 prose/OCR/table/email、数字与模态冲突、条件例外、重复改写、页脚泄漏、多对多覆盖、数量警告和零调用重校验回归；保持 MiniMax 模型、prompt 之外的业务配置、0.90 阈值及 `qwen3.7-text-embedding` 不变。

## v2.17.2 — 2026-07-29 审核爆炸与证据回查修复

- 修复文档级任意不可制卡页脚污染全部知识原子的作用域泄漏；非知识来源门禁现在只检查该原子实际解析到的证据块，缺失 locator 时不借用无关块或原因。
- 新增零 provider 的确定性本地证据修复：在 `evidence_index`、block 与解析 Markdown 中按中日文词元、字符、数字/单位和页/块提示保守对齐，仅在分数和胜出间距均满足时回填来源块的逐字原文及页/块 locator；歧义继续关闭自动批准。
- 原子验证前确定性去噪、近重复合并及严格的相邻同证据合并，保留全部 `point_ids`、原 atom id 和 evidence；不同房间、部件、数字、单位、阈值、步骤和例外不合并。
- 审核路由区分不可覆写硬门禁与仅可信度等软门禁；人工批准必须有原文证据、填写理由，并记录时间、理由及原软门禁。证据缺失、数字冲突、Schema、危险路径、无支持内容和重复不可覆写。
- 审核台改为非技术呈现：生成内容/原文依据并排、页/块定位、具体原因/风险/建议、中文门禁清单和自动修复说明；原始 JSON 默认收在“开发诊断”。指标区分候选、自动批准、待审核、硬拒绝/噪声和合并，不再把待审核记为拒绝。
- 旧 review artifact 读取时惰性迁移且保留已有状态/决策；parsed、summary、atom batch/aggregate 检查点合同不变，重跑只进行本地合并、证据协调、验证和审核路由，无需重新上传或 OCR。
...
- 设置页正常流程仅显示 API Key、明确同意与启用控制；新增固定中性探针连接测试，不读取或保存用户内容/探针向量。
- 原生响应按 `text_index` 恢复顺序并保留 usage，数量、schema、维度和非有限数错误均使用稳定脱敏代码；鉴权、限流与超时维持可控重试。
- API Key 沿用 `~/.eks-secrets.json` 本地凭据文件并原子持久化，普通设置、诊断和报告继续剥离；修复设置页输入 MiniMax/MinerU/语义密钥后重启可能丢失的问题。
- 旧 v2.16 语义配置安全迁移到固定提供商签名，只失效语义向量/建议；摄取路径、数据与解析/OCR/AI checkpoints 不受影响。
...
- 新增可选的 Qwen3-Embedding-0.6B 卡片后处理：默认关闭、默认 Shadow、默认 1024 维，必须明确同意后才会向用户配置的 OpenAI-compatible embeddings 端点发送请求。
- 嵌入载荷仅含确定性规范化的标题、分类、标签和主张摘要；路径、原始证据、秘密、诊断、原文和易变字段不会进入请求或持久队列。
- 新增模型+维度+载荷哈希缓存、独立 `semantic/vector-index.v1.json` 精确余弦索引、元数据预筛选、候选硬上限、增量 upsert/tombstone/rebuild 和 ANN-ready 查询边界。
- 新增重复候选与 related/supersedes/conflicts 审核建议；数字、单位、日期、版本、主体和证据身份守卫阻止不安全推断。结果不自动合并、删除、改状态、写关系或修改 Markdown。
- 新增非阻塞可恢复异步队列、超时/重试/退避/限流/批处理/并发/卸载取消，以及设置页和控制台运行、重建、清空、进度与脱敏指标。
- 语义设置、缓存和索引签名与解析/OCR/AI 原子 checkpoint 完全分离；升级、清空或改变模型/维度不会使既有摄取检查点失效。
# 2.20.12

- 修复 2.20.11 把生产状态机叠在旧完成/UI/路径体系上的架构问题：完成事件前必须从持久化账本重载当前 manifest，并逐条通过 Obsidian Vault API 校验和打开 `final_path`。
- `overallPercent`、`production_state`、`terminal_outcome`、UI、performance 与 diagnostic 统一来自当前运行可见 manifest；0% completed、空路径入库和三套统计漂移全部失败关闭。
- 新增唯一 `AutoDocumentParser`：Office/邮件/文本本地确定性解析；PDF 本地质量探测；必要时 MinerU；失败后本地 OCR；仍低质量则明确失败。PaddleOCR 顺序与影子评估退出生产。
- 权威输出根统一为 `06-知识库/业务库` 与 `06-知识库/招投标库`；旧顶层默认值自动迁移，旧自定义根只提示选择且不移动/删除用户文件。
- 设置页只显示自动选择说明、MinerU key 与必要云端识别隐私开关；高级设置不再暴露引擎编排。
- 新增真实故障回归、静态架构门和真实 Obsidian 双启动门，断言两类默认源目录、两库最终文件逐条可打开、重启可见且 Vault 顶层无旧目录新写入。

# 2.20.11

- 生产架构收敛为“解析/规范化 → 通用知识单元 → 质量检查/少量确认 → 两库计划 → 原子提交 → 当前运行逐文件验证”一条流程。
- 新增机器可检查的生产流程契约和五状态状态机；只有当前运行的 planned、committed、visible_verified 集合完全相同才能显示“已入库”。
- 所有生产知识 Markdown 统一通过 `ProductionCommitService` 与 `KnowledgeWritePort`；旧卡片写入和旧事务成功恢复入口在生产中明确拒绝。
- 每次处理/重试生成新 run_id；只复用解析、OCR、翻译和模型规范化中间结果，旧写入证据在启动或重试时失效。
- 两库设置统一为 `knowledgeTenderRoot` 与 `knowledgeBusinessRoot`，旧输出字段仅在加载时迁移。
- 新增架构静态门、假成功反例门，并扩展真实 Obsidian 双启动门到业务库与在办招投标库。
...
- 新增默认关闭的 Phase 3 影子审核闸：正常条目自动通过，轻微问题仅提示，只有项目归属冲突、
  ...
  才要求人工处理；不使用统一置信度门槛。
- 人工事项按文档和可执行根因分组，不合并不同事实冲突；新增非技术摘要、实际计数、文档撤回
  计划与可校验的只追加决策记录原语。本阶段不接管生产切片、不自动写入、不删除文件、不改项目状态。
# 2.19.1

- 表格切片改为按“材料/产品 + 位置/系统 + 要求组”识别可检索知识主题，保留表题、表头、单位、合并表头及页码/工作表/行列依据。
- PDF/OCR、DOCX 与 XLSX 表格在总结后增加确定性覆盖补全，过滤空行、续页重复表头及重复内容，并记录表格、主题、候选、合并、丢弃原因和密集表压缩警告。
- 审核只阻断已确认的无依据或冲突事实；生成内容与原文依据会标出具体差异，并用中文说明生成值、原文值和影响。
- 普通审核界面不再显示内部原因码、校验对象或原始 JSON；主要操作精简为“批准、重新生成、更多操作、关闭”。
- 旧审核缓存和恢复产物在入口统一归一化，避免重新出现已经匹配却同时显示通过与阻断的状态。
# 2.20.7

- 建立 universal/structured 专用审核闭环：按语义路径展示 Phase 3、关系审核和硬冲突，持久化接受、丢弃、路由修正、重规划与转人工决定，硬冲突不可强批。
