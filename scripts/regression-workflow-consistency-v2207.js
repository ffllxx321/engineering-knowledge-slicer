...
async function main() {
  ...
  const tasks = [0, 1, 22].map((count) => ({ task_id: `n${count}`, semantic_path: 'universal', status: count ? 'written' : 'completed_no_output', output_paths: Array.from({ length: count }, (_, i) => `知识/${i}.md`), verified_records: Array.from({ length: count }, (_, i) => ({ state: 'visible_verified', record_id: `bi-${i}`, record_kind: 'business_item', path: `知识/${i}.md`, content_hash: 'a'.repeat(64), transaction_id: `txn-${count}` })), written_card_ids: ['legacy-poison'], result_counts: { knowledge_records: count, verified: count, written: count, review: 0 } }));
