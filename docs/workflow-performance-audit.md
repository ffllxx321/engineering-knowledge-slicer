...
| PERF-03 | P1 | AI | `summarizeDocument` | 长文档 map/reduce；成功 chunk 暂未独立持久化 | reduce 失败时可能重复 map | 部分解决（阶段级 checkpoint）；chunk 级缓存留后续 |
| PERF-04 | P1 | polling | MinerU/PaddleOCR loops | 支持超时但 AbortSignal 未全链透传 | 取消只在进度回调边界生效 | 未完全修，风险见结果 |
...
| PERF-06 | P2 | config | `loadRuntimeContracts`/`loadTagLibraryText` | 每任务读取相同 schema/prompt/tag/folder-map | 批处理重复 vault IO | 建议按 mtime+版本缓存；本轮未冒险改变 vault 失效语义 |
| PERF-07 | P2 | UI | `refreshViews` | 关键阶段重建 Dashboard；心跳已增量 | 阶段多时仍有重排 | 已确认心跳不全刷；完整 keyed rows 留后续 |
...
| QUAL-02 | P2 | duplicate | workflow fingerprint set | 批内与历史 fingerprint Set 去重 | 避免同内容重复写卡 | 保持并由现有 identity/本轮报告确认 |
