...
- Component templates retain the compatible `{{created}}`, `{{updated}}`, and `{{event_date}}` placeholders. Their values are calendar dates (`YYYY-MM-DD`), not precise instants; precise instants stay in operational records.
