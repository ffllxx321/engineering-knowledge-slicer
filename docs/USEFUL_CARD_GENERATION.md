...
The production path is enabled unconditionally as `5.0-structure-aware-useful-card`. A canonical document now contains stable structure nodes and validated hierarchy/continuation edges. Cached `4.x`, `useful-card/1.0`, or missing-structure outputs are stale and must regenerate structure/events/plans; verified translation checkpoints remain reusable because translation text semantics and evidence hashes are unchanged.
