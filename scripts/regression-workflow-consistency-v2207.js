...
async function main() {
  ...
  const taskModule = loadBundleModule('src/core/task.js', { crypto: require('crypto'), path: require('path') });
  ...
  const tasks = [0, 1, 22].map((count) => ({ task_id: `n${count}`, semantic_path: 'universal', status: count ? 'written' : 'completed_no_output', output_paths: Array.from({ length: count }, (_, i) => `知识/${i}.md`), written_card_ids: ['legacy-poison'], result_counts: { knowledge_records: count, written: count, review: 0 } }));
  ...
  files.set(manifestPath, JSON.stringify({ status: 'committed', transaction_id: 'txn1', plan_id: 'plan1', steps: [{ action: 'create', record_kind: 'company_knowledge', path: '长期业务库/k1.md', content_hash: structured.hash(content) }] })); files.set('长期业务库/k1.md', content);
  ...
  const recovered = await recovering.recoverCommittedStructuredTasks([{ task_id: 'crash', status: 'writing', artifacts: {} }]); assert.strictEqual(recovered[0].status, 'written'); assert.strictEqual(recovered[0].structured_transaction_id, 'txn1'); assert.deepStrictEqual(recovered[0].output_paths, ['长期业务库/k1.md']);
