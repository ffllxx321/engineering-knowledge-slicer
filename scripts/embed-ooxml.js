...
const sourcePath = path.join(root, 'src', 'ooxml.js');
const moduleId = 'src/core/ooxml.js';
const startMarker = `/**\n * @module ${moduleId}\n */\n"${moduleId}": function(require, module, exports) {\n`;
const endMarker = '\n\n},\n';
const insertionMarker = '/**\n * @module src/core/block-v0\n';
function generatedModule() {
  return `${startMarker}${fs.readFileSync(sourcePath, 'utf8').replace(/\s+$/, '')}${endMarker}`;
}
function replace(bundle) {
  const start = bundle.indexOf(startMarker);
  if (start >= 0) {
    const next = bundle.indexOf(insertionMarker, start + startMarker.length);
    if (next < 0) throw new Error(`找不到 ${moduleId} 后续模块`);
    return bundle.slice(0, start) + generatedModule() + bundle.slice(next);
  }
...
const expected = replace(current);
if (process.argv.includes('--check')) {
  if (current !== expected) {
    console.error(`bundle 中的 ${moduleId} 与 src/ooxml.js 不一致；请运行 node scripts/embed-ooxml.js`);
    process.exitCode = 1;
  }
